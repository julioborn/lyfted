...
export default function PlanesPage() {
  ...
  const cargarPlanes = () => {
    const planesCargados = dataStore.getPlanes()
      ...
      {planes.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {planes.map((plan) => {
            ...
            const progreso = plan.totalDias > 0 ? Math.round((plan.diasCompletados / plan.totalDias) * 100) : 0
              ...
              <Card key={plan.id} className="hover:shadow-md transition-shadow">
                <CardContent className="pt-4 md:pt-6 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge variant={estaActivo ? "default" : "secondary"} className="text-xs shrink-0">
                  ...
                  <Button asChild className="w-full bg-transparent" size="sm" variant="outline">
                    <Link href={`/profesor/planes/${plan.id}`}>
